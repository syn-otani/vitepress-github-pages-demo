            ...
            for (let i = 0; i < envConfig.demos.length; i++) {
                ...
                var proc = gulp
                    .src([
                        ...
                        '!' + paths.src.js.files,
                        '!' + paths.src.scss.dir,
                    ], { encoding: false }).pipe(gulp.dest(destPath));
                if (i == envConfig.demos.length - 1)
                    resolve(proc);
            }
        });
...
// 必要なフォントのみコピー（woff, woff2のみ）
gulp.task('copy:fonts:minimal', function () {
    var destPath = paths.dist.base.assets + '/fonts';
    destPath = destPath.replace(paths.dist.base.dir, paths.dist.base.dir + '/' + envConfig.run);

    return gulp
        .src([
            './src/assets/fonts/remixicon.woff',
            './src/assets/fonts/remixicon.woff2',
...
// 必要な画像のみコピー
gulp.task('copy:images:minimal', function () {
    var destPath = paths.dist.base.assets + '/images';
    destPath = destPath.replace(paths.dist.base.dir, paths.dist.base.dir + '/' + envConfig.run);
...
// 必要なライブラリのみコピー
gulp.task('copy:libs:minimal', function () {
    var destPath = paths.dist.libs.dir;
    destPath = destPath.replace(paths.dist.base.dir, paths.dist.base.dir + '/' + envConfig.run);
...
// 最小CSS（RTLなし、sourcemapなし）
gulp.task('scss:minimal', function () {
    var scssFiles = paths.src.scss.main;
    scssFiles = scssFiles.replace('config', 'config/' + envConfig.run);
...
gulp.task('bootstrap:minimal', function () {
    var scssFiles = paths.src.bootstrap.files;
    scssFiles = scssFiles.replace('config', 'config/' + envConfig.run);
...
gulp.task('custom:minimal', function () {
    var scssFiles = paths.src.custom.main;
    scssFiles = scssFiles.replace('config', 'config/' + envConfig.run);
...
gulp.task('icon:minimal', function () {
    var cssDest = paths.dist.css.dir;
    cssDest = cssDest.replace(paths.dist.base.dir, paths.dist.base.dir + '/' + envConfig.run);
...
// Chemicals用最小ビルド
gulp.task('build:minimal', gulp.series(
    'clean:dist',
    gulp.parallel(
        'copy:fonts:minimal',
        'copy:images:minimal',
